...
#define I2C_SLAVE_ADDRESS            0xE2 //Temporarily define (requires redefinition)
...
int8_t bus_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data_ptr, uint16_t data_len)
{
  // Transfer structure
     ...
     for(int i = 0; i < data_len; i++)
       {
         txbuffer[i+1] = *reg_data_ptr[i];
       }

     // Initializing I2C transfer
      i2c.addr          = dev_addr;
      i2c.flags         = I2C_FLAG_WRITE;
      i2c.buf[0].data   = txbuffer;
      i2c.buf[0].len    = data_len + 1;
      i2c.buf[1].data   = NULL;
      i2c.buf[1].len    = 0;

      result = I2C_TransferInit(I2C0, &i2c);
...
int8_t bus_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data_ptr, uint16_t data_len)
{
  // transfer structure
     ...
     // Initializing I2C Transfer
     i2c.addr        = dev_addr;
     i2c.flags       = I2C_FLAG_WRITE_READ; // must write target address before reading
     i2c.buf[0].data = &reg_addr;
     i2c.buf[0].len  = 1;
     i2c.buf[1].data = reg_data_ptr;
     i2c.buf[1].len  = data_len;
